...
# rsync params.
OPTIONS="-aAXHhq --delete --exclude-from=$EXCLUDE_FILE" 
COMPRESS=false
LINK_DEST=false
SOURCE="/"
BASE_DEST=""
...
if [ "$BASE_DEST" == "" ]; then
	echo -e "Error: you must specify a destination directory.\n" > /dev/stderr
	usage
	exit 1
fi
...
# Backup the system.
rsync $OPTIONS $SOURCE $DEST &>> $LOG_FILE ||
    (raspbot_log "$0: error while backing up the system." && exit 1)
    ...
    for DATABASE in `cat $DB_NAMES_FILE | tr "\n" " "`; do
        mysqldump --defaults-file=/root/.my.cnf $DATABASE \
            > ${DB_DEST}/${DATABASE}.sql 2>> $LOG_FILE ||
                (raspbot_log "$0: error while backing up db $DB." && exit 1)
        if $COMPRESS; then
            gzip ${DB_DEST}/${DATABASE}.sql ||
                (raspbot_log "$0: error while compressing db $DB." && exit 1)
        fi
    done
